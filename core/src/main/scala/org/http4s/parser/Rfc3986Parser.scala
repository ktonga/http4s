...
private[parser] trait Rfc3986Parser {
  ...
  def Uri: Rule1[org.http4s.Uri] = rule {
    (AbsoluteUri | RelativeRef) ~ EOI
  }
  ...
  def Port = rule {
    ":" ~ (capture(oneOrMore(Digit)) ~> { s: String => (Some(s.toInt)) } | push(None)) | push(None)
  }

  def IpLiteral = rule {
    "[" ~ capture(IpV6Address | IpVFuture) ~ "]"
  }

  def IpVFuture = rule {
    "v" ~ oneOrMore(HexDigit) ~ "." ~ oneOrMore(Unreserved | SubDelims | ":")
  }
  ...
  def IpV4Address = rule {
    3.times(DecOctet ~ ".") ~ DecOctet
  }
  ...
  def RegName: Rule0 = rule {
    zeroOrMore(Unreserved | PctEncoded | SubDelims)
  }

  def Path: Rule1[String] = rule {
    (PathAbempty | PathAbsolute | PathNoscheme | PathRootless | PathEmpty) ~> { s: String => decode(s) }
  }

  def PathAbempty: Rule1[String] = rule {
    capture(zeroOrMore("/" ~ Segment))
  }

  def PathAbsolute: Rule1[String] = rule {
    capture(oneOrMore("/" ~ Segment))
  }

  def PathNoscheme: Rule1[String] = rule {
    capture(SegmentNzNc ~ zeroOrMore("/" ~ Segment))
  }

  def PathRootless: Rule1[String] = rule {
    capture(SegmentNz ~ zeroOrMore("/" ~ Segment))
  }

  def PathEmpty: Rule1[String] = rule {
    push("")
  }
  ...
  def SegmentNzNc = rule {
    oneOrMore(Unreserved | PctEncoded | SubDelims | "@")
  }
